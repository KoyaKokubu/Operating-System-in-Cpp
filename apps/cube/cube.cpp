...
extern "C" void main(int argc, char** argv) {
		...
		for (;;) {
				// rotate a cube about each x, y, z axis
				thx = (thx + 182) & 0xffff;
				thy = (thy + 273) & 0xffff;
				thz = (thz + 364) & 0xffff;
				...
				// compute 4 * z-coordinates of centers of 6 surfaces
				// ( = compute depths of surfaces) 
				...
				// At first, clear the screen, then draw a cube
...
void DrawObj(uint64_t layer_id) {
		// convert the object coordinates vert int the screen coordinates scr (deeper -> z+)
		for (int i=0; i < kCube.size(); i++) {
		...
		for (;;) {
				// draw objects in the order of depth from furthest to nearest
				...
				// draw surface whose normal vector facing forward
...
void DrawSurface(uint64_t layer_id, int sur) {
		const auto& surface = kSurface[sur]; // surface to draw
		int ymin = kCanvasSize, ymax = 0; // area to draw [ymin, ymax]
		int y2x_up[kCanvasSize], y2x_down[kCanvasSize]; // pair of Y, X corrdinates
