...
extern "C" void main(int argc, char** argv) {
		...
		for (;;) {
				// clear the screen and draw every objects
				...
				if ((ball_dx < 0 && ball_x_ < kBallRadius) ||
						(ball_dx > 0 && kCanvasWidth - kBallRadius <= ball_x_)) { // wall
					ball_dir = 180 - ball_dir;
				}
				if (ball_dy < 0 && ball_y_ < kBallRadius) { // ceil
					ball_dir = -ball_dir;
				} else if (bar_x <= ball_x_ && ball_x_ < bar_x + kBarWidth &&
										ball_dy > 0 && kBarY - kBallRadius <= ball_y_) { // bar
					ball_dir = -ball_dir;
				} else if (ball_dy > 0 && kCanvasHeight - kBallRadius <= ball_y_) { // fall
					ball_dir = 0;
					ball_y = -1;
					continue;
				}
				...
				do {
						...
						if (!blocks[index_y].test(index_x)) { // no block
							break;
						}
