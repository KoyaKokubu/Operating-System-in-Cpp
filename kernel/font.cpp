...
void InitializeFont() {
    ...
    void WriteAscii(std::shared_ptr<Window> window, Vector2D<int> pos, char c, const PixelColor& color) {
        ...
        for (int dy = 0; dy < 16; ++dy) {
            for (int dx = 0; dx < 8; ++dx) {
                if ((font[dy] << dx) & 0x80u) {
                    window->Write(pos + Vector2D<int>{dx, dy}, color);
                }
            }
        }
    }
    ...
    void WriteString(std::shared_ptr<Window> window, Vector2D<int> pos, const char* s, const PixelColor& color) {
        int x = 0;
				while (*s) {
						const auto [ u32, bytes ] = ConvertUTF8To32(s);
						WriteUnicode(window, pos + Vector2D<int>{8 * x, 0}, u32, color);
						s += bytes;
						x += IsHankaku(u32) ? 1 : 2;
				}
    }
		...
		Error WriteUnicode(std::shared_ptr<Window> window, Vector2D<int> pos,
											char32_t c, const PixelColor& color) {
				if (c <= 0x7f) {
						WriteAscii(window, pos, c, color);
						return MAKE_ERROR(Error::kSuccess);
				}

				auto [ face, err ] = NewFTFace();
				if (err) {
						WriteAscii(window, pos, '?', color);
						WriteAscii(window, pos + Vector2D<int>{8, 0}, '?', color);
						return err;
				}
				if (auto err = RenderUnicode(c, face)) {
						FT_Done_Face(face);
				...
				for (int dy=0; dy < bitmap.rows; ++dy) {
						...
						for (int dx=0; dx < bitmap.width; ++dx) {
								const bool b = q[dx >> 3] & (0x80 >> (dx & 0x7));
								if (b) {
									window->Write(glyph_topleft + Vector2D<int>{dx, dy}, color);
								}
						}
				}
