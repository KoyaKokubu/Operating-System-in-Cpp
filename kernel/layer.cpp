...
unsigned int LayerManager::GetLatestID() const {
    return latest_id_;
}
...
void ActiveLayer::Activate(unsigned int layer_id) {
    ...
    if (active_layer_ > 0) {
		...
		Log(kDebug, "active_layer_before %d -> ", active_layer_);
    ...
    if (active_layer_ > 0) {
        ...
        manager_.UpDown(active_layer_, manager_.GetHeight(mouse_layer_) - 1);
				// manager_.UpDown(active_layer_, manager_.GetHeight(mouse_layer_));
        manager_.Draw(active_layer_);
				SendWindowActiveMessage(active_layer_, 1);
    }
		Log(kDebug, "active_layer_after %d -> ", active_layer_);
}
