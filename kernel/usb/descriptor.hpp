...
namespace usb {
    struct DeviceDescriptor {
        ...
        uint8_t descripter_type;    // offset 1
    ...
    struct HIDDescriptor {
        ...
        struct ClassDescriptor {
            /** @brief type of class-specific descriptor */
            uint8_t descriptor_type;
            /** @brief byte length of class-specific descriptor*/
            uint16_t descriptor_length;
        } __attribute__((packed));

        /** @brief get info about HID-specific descriptor.
        *
        *   HID has at least one class-specific descriptor.
        *   num_descriptors indicates the number of them.
        *   Report descriptor (type=34) does exists if it is HID device,
        *   so num_descriptors is at least one.
        *
        *   @param index : fetching descriptor's number. 0 <= index < num_descriptors.
        *   @return info of descriptor specified with index. If index is out of range, return nullptr.
        */
