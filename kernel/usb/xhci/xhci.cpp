...
namespace {
    ...
    /* Root hub port must handle the port without any other processes
    *  until resetting and allocating addresses.
    *  kWaitingAddressed is the state to wait for the processes
    *  from reset (kResettingPort) to allocation of addresses (kAddressingDevice).
    */
    ...
    /** A number of the port running the processes from kResettingPort to kAddressingDevice
    *   0 indicates that there is no port of the state.
    */
    ...
    void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
        ...
        uint32_t superspeed_ports = pci::ReadConfReg(xhc_dev, 0xdc); // USB3PRM (M: mask)
        pci::WriteConfReg(xhc_dev, 0xd8, superspeed_ports); // USB#_PSSEN (EN: enable)
...
namespace usb::xhci {
    ...
    Error Controller::Initialize() {
        ...
        // Host contorller must be halted before resetting it.
        ...
        // Set "Max Slots Enabled" failed in CONFIG.
    ...
    void Initialize() {
        // find xHC by prioritizing Intel product
