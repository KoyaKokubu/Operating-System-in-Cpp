...
namespace {
    ...
    void Log(LogLevel level, const usb::HIDDescriptor& hid_desc) {
        Log(level, "HID Descriptor: relase=%0x%02x, num_desc=%d",
...
namespace usb {
    ...
    /*
    * - prepare for InitializePhase2
    * 1. fetch interface descriptor
    * 2. make class driver of the interface
    * 3. set the endpoint config that the class driver will use (id, type, packet_size, interval)
    * 4. add the endpoint into ep_configs_ : ep_configs_[num_ep_configs_] = conf;
    * 5. allocate the endpoint for the class driver (interface) : class_drivers_[conf.ep_id.Number()] = class_driver;
    * 
    * -> run InitializePhase2 (SetConfiguration)
    */
    Error Device::InitializePhase2(const uint8_t* buf, int len) {
        ...
        while (auto if_desc = config_reader.Next<InterfaceDescriptor>()) {
            ...
            if (class_driver == nullptr) {
                // non-supported device. check next interface.
                continue;
            }
    ...
    /**
    * device request : request from host. use endpoint0 called control endpoint
    * - GetDescriptor
    * - SetConfiguration
    */
