...
global GetCS    ; uint16_t GetCS(void); (CS: code segment)
GetCS:
...
CallApp:	; int CallApp(int argc, char** argv, uint16_t ss,
		...
		push r14
		push r15
		mov [r9], rsp	; save stack pointer for OS (os_stack_ptr)
		...
		push rcx	; RIP
		o64 retf
		; not come here after an application finishes
...
IntHandlerLAPICTimer:		; void IntHandlerLAPICTimer();
		...
		; construct a structure of the type of TaskContext on stack
		sub rsp, 512
		fxsave [rsp]
		push r15
		push r14
		...
		add rsp, 8*8	; ignore from CR3 to GS
		pop rax
		pop rbx
		pop rcx
		pop rdx
...
SyscallEntry:		; void SyscallEntry(void);
		...
		push rax	; save systemcall number

		mov rcx, r10
		and eax, 0x7fffffff
		mov rbp, rsp

		; prepare to run system call on stack for OS
		...
		; rbx, r12~r15 are callee-saved, so do not store from calling side
		; rax is for return value, so do not store from calling side

		mov rsp, rbp

		pop rsi		; restore systemcall number
		cmp esi, 0x80000002
		je .exit
...
ExitApp:
		...
		ret		; return to the next line of CallApp
