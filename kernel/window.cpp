...
WindowRegion Window::GetWindowRegion(Vector2D<int> pos) {
    ...
    ToplevelWindow::ToplevelWindow(int width, int height, PixelFormat shadow_format, const std::string& title)
        : Window{width, height, shadow_format}, title_{title} {
            DrawWindow(*this, title_.c_str());
    }

    void ToplevelWindow::Activate() {
        Window::Activate();
        DrawWindowTitle(*this, title_.c_str(), true);
    }

    void ToplevelWindow::Deactivate() {
        Window::Deactivate();
        DrawWindowTitle(*this, title_.c_str(), false);
    }
#endif
...
Vector2D<int> ToplevelWindow::InnerSize() const {
    ...
    void DrawWindow(PixelWriter& writer, const char* title) {
        ...
        fill_rect({0, 0},           {win_w, 1},             windowBGColor);
        fill_rect({1, 1},           {win_w - 2, 1},         windowFGColor);
        fill_rect({0, 0},           {1, win_h},             windowBGColor);
        fill_rect({1, 1},           {1, win_h - 2},         windowFGColor);
        fill_rect({win_w - 2, 1},   {1, win_h - 2},         0x848484);
        fill_rect({win_w - 1, 0},   {1, win_h},             0x000000);
        fill_rect({2, 2},           {win_w - 4, win_h - 4}, windowBGColor);
    ...
    //     // fill main box
    //     fill_rect(pos + Vector2D<int>{1, 1}, size - Vector2D<int>{2, 2}, windowBGColor);

    //     // draw border lines
    //     fill_rect(pos,                            {size.x, 1}, 0x848484);
    //     fill_rect(pos,                            {1, size.y}, 0x848484);
    //     fill_rect(pos + Vector2D<int>{0, size.y}, {size.x, 1}, 0x000000);
    //     fill_rect(pos + Vector2D<int>{size.x, 0}, {1, size.y}, 0x000000);
    // }

    void DrawWindowTitle(PixelWriter& writer, const char* title, bool active) {
        const auto win_w = writer.Width();
        uint32_t bgcolor = 0x202020;
        uint32_t fgcolor = 0x008585;
        if (active) {
            bgcolor = 0x000000;
            fgcolor = windowFGColor;
        }

        FillRectangle(writer, {3, 3}, {win_w - 6, 18}, ToColor(bgcolor));
        WriteString(writer, {24, 4}, title, ToColor(fgcolor));

        for (int y=0; y<kCloseButtonHeight; ++y) {
            for (int x=0; x < kCloseButtonWidth; ++x) {
                PixelColor c = ToColor(windowFGColor);
                if (close_button[y][x] == '@') {
                    c = ToColor(windowFGColor);
    ...
    void DrawWindow(Window& window, const char* title) {
        auto fill_rect = [&window](Vector2D<int> pos, Vector2D<int> size, uint32_t c) {
            FillRectangle(window, pos, size, ToColor(c));
        };
        const auto win_w = window.Width();
        const auto win_h = window.Height();
        ...
        DrawWindowTitle(window, title, false);
    }

    // void DrawTextbox(Window& window, Vector2D<int> pos, Vector2D<int> size) {
    //     auto fill_rect =
    //         [&window](Vector2D<int> pos, Vector2D<int> size, uint32_t c) {
    //             FillRectangle(window, pos, size, ToColor(c));
    //         };
    ...
    void DrawWindowTitle(Window& window, const char* title, bool active) {
        ...
        FillRectangle(window, {3, 3}, {win_w - 6, 18}, ToColor(bgcolor));
        WriteString(window, {24, 4}, title, ToColor(fgcolor));
        ...
        for (int y=0; y<kCloseButtonHeight; ++y) {
            for (int x=0; x < kCloseButtonWidth; ++x) {
                ...
                window.Write({win_w - 5 - kCloseButtonWidth + x, 5 + y}, c);
            }
        }
    }
...
void DrawTextbox(PixelWriter& writer, Vector2D<int> pos, Vector2D<int> size) {
    DrawTextbox(writer, pos, size, ToColor(windowBGColor), ToColor(0xc6c6c6), ToColor(0x848484));
}

void DrawTerminal(PixelWriter& writer, Vector2D<int> pos, Vector2D<int> size) {
    DrawTextbox(writer, pos, size, ToColor(terminalBGColor), ToColor(0xc6c6c6), ToColor(0x848484));
}