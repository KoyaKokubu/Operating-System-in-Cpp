...
namespace {
        ...
        for (int dy = 0; dy < kMouseCursorHeight; ++dy) {
            for (int dx = 0; dx < kMouseCursorWidth; ++dx) {
                if (mouse_cursor_shape[dy][dx] == '@') {
                    pixel_writer->Write(position + Vector2D<int>{dx, dy}, {0, 255, 255});
    ...
    void DrawMouseCursor(std::shared_ptr<Window>& window, Vector2D<int> position) {
        for (int dy = 0; dy < kMouseCursorHeight; ++dy) {
            for (int dx = 0; dx < kMouseCursorWidth; ++dx) {
                if (mouse_cursor_shape[dy][dx] == '@') {
                    window->Write(position + Vector2D<int>{dx, dy}, {0, 255, 255});
                } else if (mouse_cursor_shape[dy][dx] == '.') {
                    window->Write(position + Vector2D<int>{dx, dy}, {0, 0, 0});
                } else {
                    window->Write(position + Vector2D<int>{dx, dy}, kMouseTransparentColor);
                }
            }
        }
    }
