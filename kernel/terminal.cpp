...
Rectangle<int> Terminal::BlinkCursor() {
    ...
    return {CalcCursorPos(), {1, 15}};
}

void Terminal::DrawCursor(bool visible) {
		if (show_window_) {
				const auto color = visible ? ToColor(terminalFGColor) : ToColor(terminalBGColor);
				#ifdef WINDOW_WRITER
						FillRectangle(*window_->Writer(), CalcCursorPos(), {1, 15}, color);
				#else
						FillRectangle(*window_, CalcCursorPos(), {1, 15}, color);
				#endif
		}
}
        ...
        if (cursor_.x > 0) {
            --cursor_.x;
						if (show_window_) {
							...
							#else
									FillRectangle(*window_, CalcCursorPos(), {8, 16}, {0, 0, 0});
							#endif
						}
        ...
        if (cursor_.x < kColumns - 1 && linebuf_index_ < kLineMax - 1) {
            ...
            ++linebuf_index_;
						if (show_window_) {
							...
							#else
									WriteAscii(*window_, CalcCursorPos(), ascii, {255, 255, 255});
							#endif
						}
            ++cursor_.x;
        }
...
void Terminal::Scroll1() {
    ...
    FillRectangle(*window_->InnerWriter(),
                    {4, 4 + 16*cursor_.y}, {8*kColumns, 16}, ToColor(terminalBGColor));
}
    ...
    if (strcmp(command, "echo") == 0) {
        ...
        PrintToFD(*files_[1], "\n");
				...
				if (show_window_) {
					#ifdef WINDOW_WRITER
						FillRectangle(*window_->InnerWriter(),
													{4, 4}, {8*kColumns, 16*kRows}, ToColor(terminalBGColor));
					#else
						FillRectangle(*window_,
													{4, 4}, {8*kColumns, 16*kRows}, ToColor(terminalBGColor));
					#endif
				}
        cursor_.y = 0;
        ...
        if (!first_arg || first_arg[0] == '\0') {
				...
				if (!first_arg || first_arg[0] == '\0') {
						...
						if (!file_entry) {
								...
								char name[13];
								fat::FormatName(*file_entry, name);
								PrintToFD(*files_[2], "%s is a directory\n", name);
								exit_code = 1;
						} else {
				...
				task_manager->NewTask()
					.InitContext(TaskTerminal, reinterpret_cast<int64_t>(term_desc))
					.Wakeup();
		} else if (linebuf_[linebuf_index_-1] == '&') {
				if (--first_arg) {
						*first_arg = ' ';
				}
				linebuf_[linebuf_index_-1] = 0;
				if (linebuf_[linebuf_index_-2] == ' ') {
						linebuf_[linebuf_index_-2] = 0;
				}
				auto term_desc = new TerminalDescriptor{
						&linebuf_[0], true, false, files_
				};
				task_manager->NewTask()
					.InitContext(TaskTerminal, reinterpret_cast<int64_t>(term_desc))
					.Wakeup();
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry& file_entry,
    ...
    __asm__("cli");
		...
		int argv_len = 32; // argv = 8*32 = 256 bytes
...
void Terminal::Print(char32_t c, const PixelColor& color) {
		if (!show_window_) {
				return;
		}
    ...
    if (c == U'\n') {
        newline();
				...
				#ifdef WINDOW_WRITER
						WriteUnicode(*window_->Writer(), CalcCursorPos(), c, color);
				#else
						WriteUnicode(*window_, CalcCursorPos(), c, color);
				#endif
				++cursor_.x;
		} else {
				if (cursor_.x >= kColumns - 1) {
...
void Terminal::Print(const char* s, const PixelColor& color, std::optional<size_t> len) {
...
Rectangle<int> Terminal::HistoryUpDown(int direction) {
    ...
    Rectangle<int> draw_area{first_pos, {8*(kColumns - 3), 16}};
    #ifdef WINDOW_WRITER
        FillRectangle(*window_->Writer(), draw_area.pos, draw_area.size, ToColor(terminalBGColor));
    #else
        FillRectangle(*window_, draw_area.pos, draw_area.size, ToColor(terminalBGColor));
    #endif
    ...
    #else
        WriteString(*window_, first_pos, history, {255, 255, 255});
    #endif
    cursor_.x = linebuf_index_ + 3;
    return draw_area;
}
...
void TaskTerminal(uint64_t task_id, int64_t data) {
    ...
    // Terminal* terminal = new Terminal(task, term_desc);
		...
		terminal = std::make_unique<Terminal>(task, term_desc);
		if (show_window) {
				layer_manager->Move(terminal->LayerID(), {100, 200});
				layer_task_map->insert(std::make_pair(terminal->LayerID(), task_id));
				active_layer->Activate(terminal->LayerID());
		}
		Log(kDebug, "task_id %d: layer_id is %d\n", task_id, terminal->LayerID());
    __asm__("sti");
            ...
            case Message::kKeyPush:
                ...
                break;
						case Message::kWindowActive:
								Log(kDebug, "Before task_id %d: layer_id is %d, window_isactive: %d\n", task_id, terminal->LayerID(), window_isactive);
								window_isactive = msg->arg.window_active.activate;
								Log(kDebug, "After task_id %d: layer_id is %d, window_isactive: %d\n", task_id, terminal->LayerID(), window_isactive);
								break;
...
size_t TerminalFileDescriptor::Read(void* buf, size_t len) {
		...
		while (true) {
				...
				// if (!timer_flag && msg->type == Message::kTimerTimeout) {
				// 		Message m{Message::kTimerTimeout};
				// 		m.arg.timer.timeout = msg->arg.timer.timeout;
				// 		m.arg.timer.value = msg->arg.timer.value;
				// 		task_manager->SendMessage(term_.GetTaskId(), m);
				// 		timer_flag = true;
				// 		continue;
				// }
				...
				term_.Print(bufc, {255, 255, 255}, 1);
				term_.Redraw();
				return 1;
		}
}
...
size_t TerminalFileDescriptor::Write(const void* buf, size_t len) {
		term_.Print(reinterpret_cast<const char*>(buf), {255, 255, 255}, len);
		term_.Redraw();
		return len;
}
